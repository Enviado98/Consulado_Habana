...
function buildCard(g, isPlaceholder = false) {
  ...
  card.innerHTML = `
    <!-- Avatar flotante — sale por encima de la tarjeta -->
    ...
    <div class="gc-card-inner">
      ...
      <!-- Strip de contacto oscuro -->
      ${contactRows ? `<div class="gc-contact-strip" style="background:color-mix(in srgb,${palette.color} 60%,#000 40%)">${contactRows}</div>` : ''}

      <!-- Cuerpo blanco -->
      <div class="gc-card-body">
        <p class="gc-desc${isPlaceholder ? ' gc-desc-muted' : ' collapsed'}" id="gc-desc-${cardIdx}">${isPlaceholder ? desc : escapeHtml(desc)}</p>
        ${!isPlaceholder ? `<button class="gc-ver-mas-btn" data-target="gc-desc-${cardIdx}" onclick="gcToggleDesc(this)">Ver más ▾</button>` : ''}
        ${isPlaceholder
